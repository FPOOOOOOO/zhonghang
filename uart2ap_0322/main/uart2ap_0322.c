...
esp_netif_dhcp_status_t *dhcpstatus2;
esp_netif_ip_info_t ip_info;
struct sockaddr_in local_addr, dest_addr;
static bool s_sta_is_connected = false;

static const char *payload = "Message from ESP32 ";
static const char *payload1 = "Message from UART ";
int sock;
    ...
    for (;;)
    {
        ...
        if (xQueueReceive(uart0_queue, (void *)&event, (portTickType)portMAX_DELAY))
        {
            ...
            case UART_DATA:
                ...
                //esp_wifi_80211_tx(WIFI_IF_AP,dtmp,event.size,true);
                //ret =esp_wifi_internal_tx(WIFI_IF_AP, (void*) dtmp, event.size);
                // int err = sendto(sock, dtmp, event.size, 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
                // if (err < 0)
                // {
...
void app_main(void)
{
    ...
    // int rettt = esp_netif_dhcps_stop(ap_netif);
    // printf("stop dhcp ret = %x\n",rettt);
