/* eth2ap (Ethernet to Wi-Fi AP packet forwarding) Example
   ...
   CONDITIONS OF ANY KIND, either express or implied.
    ...
    0707:修改为UART透传、SPI透传，屏蔽ETH透传,因为没有从ETH拿mac了，所以s_ethmac要写死,增加了wifimsgtype,SPI sendbuf 改为全局变量
    0719:改成AP-STA，单跳
*/
#include <string.h>
#include <stdio.h>
...
static bool s_sta_is_connected = false; //self as sta
static bool sta_is_connected = false;   //self as ap
...
static esp_err_t pke_wifi2ap(void *buffer, uint16_t len, void *eb)
{
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    else if (event_id == WIFI_EVENT_AP_STACONNECTED)
    {
        ...
        if (!s_con_cnt)
        {
            ...
            WIFI_INIT_DONE = true;
            esp_wifi_internal_reg_rxcb(WIFI_IF_AP, pkt_wifi2ap);
        }
        s_con_cnt++;
...
static void initialize_wifi(void)
{
    ...
    ESP_ERROR_CHECK(esp_wifi_set_storage(WIFI_STORAGE_RAM)); // this is only for wifi
    ...
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_APSTA));                 // default wifi_mode_ap
    ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_sta_config)); // default wifi_if_sta
    ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_AP, &wifi_ap_config));   // default wifi_if_sta
