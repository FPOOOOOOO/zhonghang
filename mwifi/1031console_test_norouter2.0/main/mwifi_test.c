...
// 1031:在norouter的基础上增加1.4G，在1.0的基础上改成和eth2meshroot一样的收发函数
...
static void mesh_iperf_client_task(void *arg)
{
         ...
         xTaskGetTickCount() < end_ticks && !g_mesh_iperf_cfg.finish; ++total_count)
    {
        ...
        {
            // MDF_LOGI("I am Root");
            ret = mwifi_root_write(g_mesh_iperf_cfg.addr, 1, &data_type, buffer,
                                   g_mesh_iperf_cfg.packet_len, true);
        }
        else
        {
            ret = mwifi_write(Rootaddr, &data_type, buffer,
                                  g_mesh_iperf_cfg.packet_len, true);
        }
    ...
    do
    {
        if (IsRoot == 1)
        {
            ret = mwifi_root_write(g_mesh_iperf_cfg.addr, 1, &data_type, buffer, 1, true);
        }
        else
        {

            ret = mwifi_write(Rootaddr, &data_type, buffer, 1, true);
        }
...
static void mesh_iperf_server_task(void *arg)
{
         ...
         !g_mesh_iperf_cfg.finish;)
    {
        ...
        else if (data_type.protocol == IPERF_BANDWIDTH_STOP)
        {
            ...
            if (IsRoot == 1)
            {
                ret = mwifi_root_write(g_mesh_iperf_cfg.addr, 1, &data_type, &ret, 1, true);
            }
            else
            {
                ret = mwifi_write(Rootaddr, &data_type, &ret, 1, true);
            }
...
static esp_err_t mesh_iperf_func(int argc, char **argv)
{
    ...
    if ((mesh_iperf_args.client->count && mesh_iperf_args.server->count) || (!mesh_iperf_args.client->count && !mesh_iperf_args.server->count))
    {
