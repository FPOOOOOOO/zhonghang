...
static const char *payload = "This is msg from Server";
static const char *UDPServerPing = "This is UDP Server Ping";
static const char *UDPClientPing = "This is UDP Client Ping";
...
int sock;
static bool UDPtaskcreate = false;
static bool UDPinitdone = false;
static bool ETHCallBack = false;
// UDP全局变量
...
// pv_udp_socket_select_struct_t udp_server;

// Forward packets from Wi-Fi to Ethernet
static esp_err_t pkt_wifi2eth(void *buffer, uint16_t len, void *eb)
{
    ESP_LOGI(TAG, "The msg 's LEN : %d", len);
    ESP_LOGI(TAG, "Bugger addr  : %p", buffer);
    ESP_LOGI(TAG, "eb addr  : %p", eb);
    if (s_sta_is_connected)
    {
        // bufferback=memcpy(,buffer,len)
    ...
    for (;;)
    {
        ...
        if (xQueueReceive(uart0_queue, (void *)&event, (portTickType)portMAX_DELAY))
        {
            ...
            case UART_DATA:
                ...
                //int ret = esp_wifi_80211_tx(WIFI_IF_AP,uart_recv_data,event.size,true);
                //int ret = esp_wifi_internal_tx(WIFI_IF_AP, (void *)uart_recv_data, event.size);
                Send_UDP(uart_recv_data);
                // int err = sendto(sock, uart_recv_data, event.size, 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
                // // int err = sendto(sock, payload, strlen(payload), 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
                // if (err < 0)
                // {
                //     ESP_LOGE(TAG, "Error occurred during sending: errno %d", errno);
...
static void udp_server_task(void)
{
    ...
    // The dest info
    dest_addr.sin_addr.s_addr = inet_addr("192.168.4.7");
    ...
    while (1)
    {
        ...
        while (1)
        {
            if (!UDPinitdone)
            { //建立UDP链接，让双方拿到对方的Mac
                ESP_LOGI(TAG, "UDPServer waiting for client test");
                ...
                else
                {
                    ...
                    if (strcmp(rx_buffer, UDPClientPing) == 0)
                    {
                        UDPinitdone = true;
                        for (int j = 0; j < 5; j++)
                        {
                            int err = sendto(sock, UDPServerPing, strlen(UDPServerPing), 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
                            if (err < 0)
                            {
                                ESP_LOGE(TAG, "Error occurred during sending: errno %d", errno);
                                break;
